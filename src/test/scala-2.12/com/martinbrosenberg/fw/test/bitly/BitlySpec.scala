...
/** Test Bitly's branded short URLs for CNN and Buzzfeed. This is exceptionally challenging because Bitly has a
  * Selenium-only bug if you don't wait before clicking the "shorten" button, and Buzzfeed has long-hanging AJAX.
  */
class BitlySpec extends BaseSpec {

  // Not a fan of having to `new` things in a spec, even with the `get` sugar. todo Implement DI.
  val bhp = new BitlyHome

  // Necessary to DRY up tests, but gross. todo Do this better after implementing DI.
  private def getShortUrl(originalUrl: String): String = {
    driver get bhp
    bhp.field sendKeys originalUrl + timestamp

    // this will fail without waiting... but only when doing it with Selenium *scratches head*
    // todo figure out what's making it fail, to avoid thread sleeping
    Thread sleep 2000
    bhp.button click()

    waits.until(!(bhp.field attribute "value" getOrElse "" startsWith originalUrl))
  ...
  "The shortened URL goes to the correct full URL for" - {
    testCases foreach { case (site, original, _) =>
      site in {
        ...
        driver.getCurrentUrl shouldBe fullUrl
      }
    }
  }
